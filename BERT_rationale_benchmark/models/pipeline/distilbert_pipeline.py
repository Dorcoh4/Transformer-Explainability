...
def scores_per_word_from_scores_per_token(input, tokenizer, input_ids, scores_per_id):
    ...
    for inp in input:
        ...
        score_per_word.append(np.max([item.detach().cpu() for item in score_per_char[start_idx:end_idx]]))
...
def bert_tokenize_doc(doc: List[List[str]], tokenizer, special_token_map) -> Tuple[
...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    if os.path.exists(epoch_save_file):
        ...
        # explainability
        explanations = Generator(test_classifier, masker, tokenizer)
        explanations_orig_lrp = Generator(orig_lrp_classifier, masker, tokenizer)
        method = "distilbert"
        ...
        for i in range(5, 85, 5):
            result_files.append(
                open(os.path.join(args.output_dir, '{0}/identifier_results_{1}.json').format(method_folder[method], i),
                     'w'))
...
\end{document}
                ...
                cam = cam_target
                cam = weight_map.my_scores_per_word_from_scores_per_token(tokenizer, cam, input_ids)
                print(cam)
                j = j + 1
                doc_name = extract_docid_from_dataset_element(s)
